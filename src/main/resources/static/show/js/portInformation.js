...
avalon.ready(function(){
	...
	var catIdOne = paramsMap.catIdOne;
	var pressId = paramsMap.pressId;
	if(typeof (catIdOne) != "undefined" && typeof (pressId) != "undefined"  ){
        getContent(pressId, catIdOne);
	}
});
...
var vm = avalon.define({
	...
	viewDetailDiscard : function(pressId,isRelDownload){
        $.ajax({
    ...
    viewDetail : function(pressId,isRelDownload){
        var url = window.location.pathname + "?moduleId="+paramsMap.moduleId + "&catIdOne="+vm.nowParentId+ "&pressId=" + pressId;
...
function getContent(pressId,catIdOne){
    $.ajax({
        url : "../view/queryPressDetail",
        type : 'post',
